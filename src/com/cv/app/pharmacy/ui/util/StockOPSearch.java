 ...
 * @author WSwe
 */
public class StockOPSearch extends javax.swing.JPanel implements SelectionObserver {

    static Logger log = Logger.getLogger(StockOPSearch.class.getName());
    private AbstractDataAccess dao;
    private Object parent;
    private SelectionObserver observer;
    private int selectedRow = -1;
    //private List<StockOpeningHis> listOpening = new ArrayList<StockOpeningHis>();
    //private List<Medicine> listMedicine = ObservableCollections.observableList(new ArrayList<Medicine>());
    private CodeTableModel tblMedicineModel = new CodeTableModel(Global.dao, true, "StkOpSearch");
    private StockOpSearchTableModel vouTableModel = new StockOpSearchTableModel();
    private TableRowSorter<TableModel> sorter;
    private VouFilter vouFilter;
    private int mouseClick = 2;

    /**
     * Creates new form StockOPSearch
     */
    public StockOPSearch(Object parent, SelectionObserver observer, AbstractDataAccess dao) {
        initComponents();
        ...
        try {
            //tblMedicineModel = new CodeTableModel(dao, true, "StkOpSearch");
            dao.open();
            initCombo();
            dao.close();
        } catch (Exception ex) {
            log.error("StockOPSearch : " + ex.getStackTrace()[0].getLineNumber() + " - " + ex.toString());
        }
    ...
    private void initTableVoucher() {
        ...
        tblVoucher.getColumnModel().getColumn(3).setPreferredWidth(30);
    ...
    private String getHSQL() {
        /*String strSql = "select distinct v from StockOpeningHis v join v.listDetail h where v.opDate between '"
                + DateUtil.toDateStrMYSQL(txtFromDate.getText()) + "' and '"
                + DateUtil.toDateStrMYSQL(txtToDate.getText()) + "'";*/
        String strSql = "select distinct soh.op_date, soh.op_id, l.location_name, u.user_name\n"
                + "from stock_op_his soh\n"
                + "join stock_op_detail_his sod on soh.op_id = sod.op_id \n"
                + "join location l on soh.location = l.location_id\n"
                + "left join appuser u on soh.created_by = u.user_id\n"
        ...
        if (cboLocation.getSelectedItem() instanceof Location) {
            ...
            strSql = strSql + " and soh.location = " + location.getLocationId();
        ...
        if (cboSession.getSelectedItem() instanceof Session) {
            ...
            strSql = strSql + " and soh.session_id = " + session.getSessionId();
        ...
        if (txtVouNo.getText() != null && !txtVouNo.getText().isEmpty()) {
            strSql = strSql + " and soh.op_id = '" + txtVouNo.getText() + "'";
        }
        ...
        if (listMedicine.size() > 1) {
            strSql = strSql + " and sod.med_id in (";
            String medList = "";
        ...
        try {
            ...
            if (rs != null) {
                while (rs.next()) {
                    ...
                    vs.setInvNo(rs.getString("op_id"));
                    vs.setLocation(rs.getString("location_name"));
                    vs.setTranDate(rs.getDate("op_date"));
    ...
    private void select() {
        if (selectedRow >= 0) {
            ...
            JOptionPane.showMessageDialog(Util1.getParent(), "Please select stock opening.",
