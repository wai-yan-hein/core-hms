 ...
 * @author WSwe
 */
public class SaleTableCodeCellEditor1 extends AbstractCellEditor implements TableCellEditor {

    static Logger log = Logger.getLogger(SaleTableCodeCellEditor1.class.getName());
    ...
    public SaleTableCodeCellEditor1(final AbstractDataAccess dao) {
        this.dao = dao;
    ...
    public Component getTableCellEditorComponent(JTable table, Object value,
        ...
        //VMedicine1 tmp = (VMedicine1)listItem.get(2);
        //log.info("med_id : " + tmp.getBarcode());
    ...
    private void getItem() {
        try {
            ...
            if (dao.getRowCount("select count(*) from item_type_mapping where group_id =" + Global.loginUser.getUserRole().getRoleId()) > 0) {
                Global.listItem = dao.findAllHSQL(
                        "select o from VMedicine1 o where o.active = true and o.medTypeId in (select a.key.itemType.itemTypeCode from ItemTypeMapping a) order by o.shortName");
            } else {
                if (strCodeFilter.equals("Y")) {
                    Global.listItem = dao.findAllHSQL(
                            "select o from VMedicine1 o where o.medId in (select a.key.itemId from "
                            + "LocationItemMapping a where a.key.locationId = "
                            + locationId.toString() + ") order by o.shortName");
                } else {
                    Global.listItem = dao.findAllHSQL("select o from VMedicine1 o where active = true order by o.shortName");
                }
            }
