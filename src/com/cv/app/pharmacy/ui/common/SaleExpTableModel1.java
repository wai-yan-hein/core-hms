 ...
 * @author WSwe
 */
public class SaleExpTableModel1 extends AbstractTableModel {

    static Logger log = Logger.getLogger(SaleExpTableModel1.class.getName());
    private List<SaleExpense> listExpense;
    private final String[] columnNames = {"Date", "Expense Type", "Amt-Out"};
    private String deletedList;
    private Date saleDate;

    public SaleExpTableModel1() {
    }
    ...
    public void addEmptyRow() {
        if (listExpense != null) {
            ...
            if (saleDate == null) {
                ...
                record.setExpenseDate(saleDate);
            }
    ...
    public void delete(int row) {
        ...
        if (NumberUtil.NZeroL(record.getSaleExpenseId()) > 0) {
            if (deletedList == null) {
                deletedList = record.getSaleExpenseId().toString();
            } else {
                deletedList = deletedList + "," + record.getSaleExpenseId().toString();
            }
        }
    ...
    public Double getTotal() {
        if (listExpense == null) {
            return 0.0;
        } else if (listExpense.isEmpty()) {
            return 0.0;
        } else {
            Double totalAmt = 0.0;
            totalAmt = listExpense.stream().map(se -> se.getExpAmount())
                    .reduce(totalAmt, (accumulator, _item) -> accumulator + NumberUtil.NZero(_item));

            return NumberUtil.roundTo(totalAmt, 0);
        }
    }
