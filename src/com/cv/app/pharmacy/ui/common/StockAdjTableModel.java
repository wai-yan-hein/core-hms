...
public class StockAdjTableModel extends AbstractTableModel {
    ...
    private float getBalance(String locId, String medId) {
        ...
        strSql = "select * from (\n"
                ...
                + "     where sodh.med_id = prm_med_id and soh.location = prm_location_id and soh.op_date = var_op_date\n"
                ...
                + "       and date(sh.sale_date) between var_op_date and v_tran_date \n"
                ...
                + "       and date(ph.pur_date) between var_op_date and v_tran_date \n"
                ...
                + "       and date(rih.ret_in_date) between var_op_date and v_tran_date \n"
                ...
                + "       and date(roh.ret_out_date) between var_op_date and v_tran_date \n"
                ...
                + "       and date(th.tran_date) between var_op_date and v_tran_date and tdh.med_id = prm_med_id\n"
                ...
                + "       and date(ah.adj_date) between var_op_date and v_tran_date and adh.med_id = prm_med_id\n"
                ...
                + "	  and srh.deleted = false and date(srh.receive_date) between var_op_date and v_tran_date and srdh.order_med_id = prm_med_id\n"
                ...
                + "	  and sih.deleted = false and date(sih.issue_date) between var_op_date and v_tran_date \n"
                ...
                + "	 from v_damage vd\n"
                + "	where vd.deleted = false and date(vd.dmg_date) between var_op_date and v_tran_date \n"
                + "	  and vd.med_id = prm_med_id and vd.location = prm_location_id\n"
                + "	group by vd.location, vd.med_id, expire_date\n"
                + "     union all \n"
                + "    select mu.location_id, mu.med_id, null as exp_date, sum(ifnull(mu.qty_smallest,0)*-1) ttl_qty\n" 
                + "      from med_usaged mu \n" 
                + "     where mu.location_id = prm_location_id and mu.med_id = prm_med_id\n" 
                + "       and date(mu.created_date) between var_op_date and v_tran_date\n" 
        ...
        strSql = strSql.replace("var_op_date", "'" + medOpDate + "'")
                ...
                .replace("v_tran_date", "'" + mysqlStockDate + "'");
        float ttlBalance = 0f;
        try {
            ResultSet rs = dao.execSQL(strSql);
            if (rs != null) {
