 ...
 * @author WSwe
 */
public class Purchase1 extends javax.swing.JPanel implements SelectionObserver, FormAction,
        MedInfo, KeyPropagate, KeyListener {

    static Logger log = Logger.getLogger(Purchase1.class.getName());
    ...
    private PurchaseTableModel1 purTableModel = new PurchaseTableModel1(listDetail, dao,
            medUp, this);
        ...
        try {
            ...
            switch (source.toString()) {
                ...
                case "PurVouList":
                    ...
                    dao.close();
                    currPurVou.setCustomerId(trader);
                    getTraderLastBalance();
                    calculateTotalAmount();
    ...
    private void initPurTable() {
        ...
        tblPurchase.getColumnModel().getColumn(5).setPreferredWidth(30);//Pur price
        tblPurchase.getColumnModel().getColumn(6).setPreferredWidth(20);//Discount1
        tblPurchase.getColumnModel().getColumn(7).setPreferredWidth(20);//FOC
        tblPurchase.getColumnModel().getColumn(8).setPreferredWidth(20);//Expense %
        tblPurchase.getColumnModel().getColumn(9).setPreferredWidth(30);//Expense
        tblPurchase.getColumnModel().getColumn(10).setPreferredWidth(30);//Unit cost
        tblPurchase.getColumnModel().getColumn(11).setPreferredWidth(50);//Amount

        tblPurchase.getColumnModel().getColumn(4).setCellRenderer(rightRenderer);
        tblPurchase.getColumnModel().getColumn(7).setCellRenderer(rightRenderer);

        addPurTableModelListener();

        //Change JTable cell editor
        tblPurchase.getColumnModel().getColumn(0).setCellEditor(codeCellEditor);
        tblPurchase.getColumnModel().getColumn(3).setCellEditor(new BestTableCellEditor());
        tblPurchase.getColumnModel().getColumn(4).setCellEditor(new BestTableCellEditor());
        tblPurchase.getColumnModel().getColumn(5).setCellEditor(new BestTableCellEditor());
    ...
    private void getTraderLastBalance() {
        ...
        if (strTodayDateTime != null) {
            if (!strTodayDateTime.isEmpty()) {
                ...
                if (currPurVou.getCustomerId() != null) {
                    strTrdOpt = currPurVou.getCustomerId().getTraderId().substring(0, 3);
                }
                ...
                for (int i = 0; i < listOut.size(); i++) {
                    ...
                    if (po.getItemId().getMedId().equals(medId)
                        ...
                        if (deleteOutstandList == null) {
                            deleteOutstandList = po.getOutsId().toString();
                        } else {
                            deleteOutstandList = deleteOutstandList + "," + po.getOutsId().toString();
                        }
