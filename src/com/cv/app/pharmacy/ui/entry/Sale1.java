 ...
 * @author WSwe
 */
public class Sale1 extends javax.swing.JPanel implements SelectionObserver, FormAction,
        MedInfo, KeyPropagate, KeyListener {

    static Logger log = Logger.getLogger(Sale1.class.getName());
    ...
    private SaleTableModel1 saleTableModel = new SaleTableModel1(listDetail, dao,
            medUp, this, this);
    ...
    public Sale1() {
        ...
        if (cusType.isEmpty()) {
            ...
            saleTableModel.setCusType(cusType);
        }
        applySecurityPolicy();
            ...
            case "CustomerList":
                ...
                if (cus != null) {
                    ...
                    if (tmpCus instanceof Customer) {
                        ...
                        if (creditDay != 0) {
                            if (Util1.getPropValue("system.sale.duedate.checkweekday").equals("Y")) {
                                Date tmpDate = DateUtil.toDate(DateUtil.subDateTo(DateUtil.toDate(txtSaleDate.getText()), creditDay));
                                Calendar c = Calendar.getInstance();
                                c.setTime(tmpDate);
                                int dayOfWeek = c.get(Calendar.DAY_OF_WEEK);
                                log.info("Week Day : " + dayOfWeek);
                                if (dayOfWeek == Calendar.SUNDAY) {
                                    txtDueDate.setText(DateUtil.subDateTo(tmpDate, 1));
                    ...
                    if (cus.getTypeId() != null) {
                        saleTableModel.setCusType(cus.getTypeId().getDescription());
                    } else {
                        saleTableModel.setCusType("N");
                    }
                ...
                if (!Util1.getPropValue("system.app.usage.type").equals("Hospital")) {
                    if (currSaleVou.getCustomerId() != null) {
                        ...
                        if (currSaleVou.getCustomerId().getTypeId() != null) {
                            saleTableModel.setCusType(currSaleVou.getCustomerId().getTypeId().getDescription());
                        }
                    }
 ...
 /*try {
    ...
    public void print() {
        ...
        if (isValidEntry() && saleTableModel.isValidEntry() && expTableModel.isValidEntry()) {
            ...
            if (propValue.equals("Y")) {
                ...
                Customer cus = (Customer) currSaleVou.getCustomerId();
                if (cus.getTraderGroup() != null) {
            ...
            params.put("IMAGE_PATH", Util1.getAppWorkFolder()
                    + Util1.getPropValue("report.folder.path"));
            if (Util1.getPropValue("system.sale.cus.group.report").equals("Y") && chkVouComp.isSelected()) {
    ...
    private void getTraderTransaction() {
        ...
        if (trader.getTraderId().substring(0, 3).contains("SUP")) {
            strTrdOpt = "SUP";
        } else {
            strTrdOpt = "CUS";
        }
    ...
    private void getTraderLastBalance() {
        ...
        if (!strTodayDateTime.isEmpty()) {
            String strTrdOpt;
            if (currSaleVou.getCustomerId().getTraderId().substring(0, 3).equals("SUP")) {
                strTrdOpt = "SUP";
            } else {
                strTrdOpt = "CUS";
            }
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                try {
                    ...
                    if (machine.getActionStatus() != null) {
                        ...
                        if (dao.getRowCount("select count(*) from item_type_mapping where group_id =" + Global.loginUser.getUserRole().getRoleId()) > 0) {
                            Global.listItem = dao.findAll("VMedicine1", "active = true and medTypeId in (select a.key.itemType.itemTypeCode from ItemTypeMapping a)");
        ...
        if (cus.getTraderGroup() != null) {
            ...
            if (overDueVouCnt > 0) {
                String strSql = "select count(b.vou_no) ttl_cnt from (\n"
                        ...
                        + "	 where bal > 0 and vob.trader_id = '" + cus.getTraderId() + "'\n"
                        + " union all "
                        ...
                        + "where sh.deleted = false and sh.cus_id = '" + cus.getTraderId() + "' \n"
                        + "group by sh.sale_inv_id, sh.sale_date,sh.vou_total, sh.paid_amount, sh.discount, sh.balance) a\n"
                        + "where a.balance > 0 and a.bal > 0 order by a.sale_date, a.vou_no) b\n"
                        + "WHERE b.ttl_overdue > 0";
                try {
                    ResultSet rs = dao.execSQL(strSql);
                    if (rs != null) {
                        if (rs.next()) {
                            int ttlCnt = rs.getInt("ttl_cnt");
                            if (ttlCnt > 0) { // Customer count
                                status = true;
                                JOptionPane.showMessageDialog(Util1.getParent(),
                                        "Overdue voucher have. You cannot open new voucher for this customer.",
                                        "Overdue", JOptionPane.ERROR_MESSAGE);
                            } else { // Customer group count
                                rs.close();
                                dao.close();
                                strSql = "select count(b.vou_no) ttl_cnt from (\n"
                                        + "select a.*, if(a.ttl_overdue<0,0,a.ttl_overdue) as ttl_overdue1 from (\n"
                                        + "select vob.tran_date sale_date, vob.vou_no, vob.trader_id cus_id, \n"
                                        + "		   t.trader_name, vob.vou_type, date_add(vob.tran_date, interval ifnull(t.credit_days,0) day) due_date,\n"
                                        + "		   'Opening' ref_no, vob.vou_total, vob.paid_amount ttl_paid, vob.discount, vob.balance,\n"
                                        ...
                                        + "	  join trader t on vob.trader_id = t.trader_id\n"
                                        + "	  left join customer_group cg on t.group_id = cg.group_id\n"
                                        + "	 where bal > 0 and t.group_id = '" + cus.getTraderGroup().getGroupId() + "' "
                                        + " union all "
                                        + "select sh.sale_date, sale_inv_id vou_no, sh.cus_id, t.trader_name, 'SALE' vou_type,\n"
                                        + "       sh.due_date, sh.remark ref_no, sh.vou_total, (sh.paid_amount+ifnull(pah.pay_amt,0)) as ttl_paid, sh.discount, sh.balance,\n"
                                        + "	   sum(ifnull(balance,0))-(ifnull(pah.pay_amt,0)) bal, \n"
                                        ...
                                        + "				and pv.vou_type = 'SALE'\n"
                                        + "			  group by pv.vou_no, pv.vou_type) pah on sh.sale_inv_id = pah.vou_no\n"
                                        + "where sh.deleted = false and (t.group_id = '" + cus.getTraderGroup().getGroupId() + "') \n"
                                        + "group by sh.sale_inv_id, sh.sale_date,sh.vou_total, sh.paid_amount, sh.discount, sh.balance) a\n"
                                        + "where a.balance > 0 and a.bal > 0 order by a.sale_date, a.vou_no) b\n"
                                        + "WHERE b.ttl_overdue > 0";
                                ...
                                rs = dao.execSQL(strSql);
                                if (rs.next()) {
                                    int ttlGroupCnt = rs.getInt("ttl_cnt");
                                    if (ttlGroupCnt >= overDueVouCnt) {
                                        status = true;
                                        JOptionPane.showMessageDialog(Util1.getParent(),
                                                "Group overdue count is exceeded. You cannot open new voucher for this customer group.",
                                                "Overdue", JOptionPane.ERROR_MESSAGE);
                                    }
                                }
    ...
    private void initComponents() {
        ...
        jPanel4Layout.setVerticalGroup(
            jPanel4Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
            .add(jPanel4Layout.createSequentialGroup()
                .add(jScrollPane6, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 109, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
                .addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
