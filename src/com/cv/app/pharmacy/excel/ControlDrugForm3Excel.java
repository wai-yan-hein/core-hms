 ...
 * @author winswe
 */
public class ControlDrugForm3Excel extends GenExcel {

    static Logger log = Logger.getLogger(ControlDrugForm3Excel.class.getName());
    private final AbstractDataAccess dao;
    private final String fileName;

    public ControlDrugForm3Excel(AbstractDataAccess dao, String fileName) {
        super(dao);
        this.dao = dao;
    ...
    public void genExcel() {
        String strSql = "select tran_date, item_id, item_name, tsio.location_id, location_name,\n"
                ...
                + "where user_id = $P{user_id} and (sale_qty <> 0 or out_ttl <> 0) order by tran_id, item_id, tran_date ";

        strSql = strSql.replace("$P{user_id}", "'" + getUserId() + "'");
        try {
            List<String> listHeader = new ArrayList();
            listHeader.add("ေဆးဆိုုင္က ၀ယ္ယူသည့္ ေရာင္းခ်သည့္ ရက္စြ");
            ...
            POIUtil.genExcelFileForControlDrugForm3(listHeader, listField, hmType, rs, fileName);
        } catch (Exception ex) {
