...
import com.cv.app.ot.ui.common.OTVouTableModel;
 ...
 * @author WSwe
 */
public class OTVouSearchDialog extends javax.swing.JDialog implements SelectionObserver {

    static Logger log = Logger.getLogger(OTVouSearchDialog.class.getName());
    private final AbstractDataAccess dao = Global.dao;
    private SelectionObserver observer;
    private VouFilter vouFilter;
    private int selectedRow = -1;
    private TableRowSorter<TableModel> sorter;
    private OTServiceFilterTableModel srvFilterTableModel
            = new OTServiceFilterTableModel(dao, true, "OTVoucherSearch");
    private OTVouTableModel vouTableModel = new OTVouTableModel();
     ...
     */
    public OTVouSearchDialog(SelectionObserver observer, String option) {
    ...
    private void getPrvFilter() {
        VouFilter tmpFilter = (VouFilter) dao.find("VouFilter", "key.tranOption = 'OTVouSearch'"
                + " and key.userId = '" + Global.loginUser.getUserId() + "'");

        if (tmpFilter == null) {
            vouFilter = new VouFilter();
            vouFilter.getKey().setTranOption("OTVouSearch");
    ...
    private void initTable() {
        List<OTVouSrvFilter> listVouSrvFilter = dao.findAll("OTVouSrvFilter",
                "key.tranOption = 'OTVoucherSearch' and key.userId = '"
    ...
    private String getHSQL() {
        /*String strSql = "select distinct v from OTHis v join v.listOPDDetailHis h where date(v.invDate) between '"
                + DateUtil.toDateStrMYSQL(txtFrom.getText()) + "' and '"
                + DateUtil.toDateStrMYSQL(txtTo.getText()) + "'";*/
        String strSql = "select distinct date(sh.ot_date) ot_date, sh.ot_inv_id, sh.remark, \n"
                + "sh.patient_id cus_id, sh.patient_name cus_name, "
                + "usr.user_name, sh.vou_total, sh.deleted\n"
                + "from ot_his sh\n"
                + "join ot_details_his sdh on sh.ot_inv_id = sdh.vou_no\n"
                + "join ot_service med on sdh.service_id = med.service_id\n"
                + "join appuser usr on sh.created_by = usr.user_id\n"
                + "where date(sh.ot_date) between '" + DateUtil.toDateStrMYSQL(txtFrom.getText()) + "' and '"
                + DateUtil.toDateStrMYSQL(txtTo.getText()) + "'";
        ...
        strSql = strSql + " order by date(sh.ot_date) desc, sh.ot_inv_id desc";
    ...
    private void initComponents() {
        ...
        cboMachine = new javax.swing.JComboBox<String>();
        jLabel10 = new javax.swing.JLabel();
        cboUser = new javax.swing.JComboBox<String>();
