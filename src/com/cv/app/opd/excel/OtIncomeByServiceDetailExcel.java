 ...
 * @author winswe
 */
public class OtIncomeByServiceDetailExcel extends GenExcel {

    static Logger log = Logger.getLogger(OtIncomeByServiceDetailExcel.class.getName());
    private final AbstractDataAccess dao;
    private final String fileName;

    public OtIncomeByServiceDetailExcel(AbstractDataAccess dao, String fileName) {
        super(dao);
        this.dao = dao;
    ...
    public void genExcel() {
        String strSql = "select tmp.group_name,tmp.service_name,tmp.patient_name,sum(tmp.ttl_qty1) ttl_qty,tmp.admission_no,\n"
                ...
                + "((select oc.group_name, os.service_name,oh.patient_name, sum(ifnull(odh.qty,0)) ttl_qty1,oh.admission_no, sum(ifnull(odh.amount,0)) ttl_amount1,d.doctor_name,\n"
                ...
                + "from ot_his oh, ot_details_his odh, ot_service os, ot_group oc, tmp_ot_service_filter tosf,doctor d\n"
                + "where oh.ot_inv_id = odh.vou_no and oh.doctor_id = d.doctor_id and odh.ot_detail_id not in (select ot_detail_id from ot_dr_fee_join)\n"
                ...
                + "group by oc.group_name, os.service_name,d.doctor_name,oh.patient_name,oh.admission_no\n"
                + "order by d.doctor_name,oc.group_name, os.service_name,oh.patient_name)\n"
                + "union\n"
                + "(select oc.group_name, os.service_name,oh.patient_name,sum(ifnull(odh.qty,0)) ttl_qty1,oh.admission_no,\n"
                ...
                + "sum(ifnull(odh.qty, 0)*ifnull(odh.srv_fee4,0)) srv_fee_amt4,\n"
                + "sum(ifnull(odh.qty, 0)*ifnull(odh.srv_fee5,0)) srv_fee_amt5\n"
                + "from ot_his oh, ot_details_his odh ,ot_doctor_fee ddf,ot_dr_fee_join ddfj,ot_service os, ot_group oc, tmp_ot_service_filter tosf,\n"
                + "doctor d\n"
                + "where oh.ot_inv_id = odh.vou_no and ddfj.ot_detail_id = odh.ot_detail_id  and odh.service_id = tosf.service_id and tosf.user_id = $P{user_id}\n"
                + "and ddfj.dr_fee_id = ddf.dr_fee_id and odh.service_id = os.service_id and os.group_id = oc.group_id  and d.doctor_id = ddf.doctor_id\n"
                ...
                + "and (oh.patient_id = $P{reg_no} or $P{reg_no} = '-')\n"
                + "group by oc.group_name, os.service_name,d.doctor_name,oh.patient_name,oh.admission_no\n"
                + "order by d.doctor_name,oc.group_name, os.service_name,oh.patient_name)) tmp\n"
                + "group by tmp.group_name, tmp.service_name,tmp.doctor_name,tmp.patient_name,tmp.admission_no\n"
                + "order by tmp.doctor_name,tmp.group_name, tmp.service_name,tmp.patient_name";
        ...
        try {
            ...
            listHeader.add("OT Service Name");
            listHeader.add("Qty");
            listHeader.add("Amount");
            listHeader.add("Hospital");
            listHeader.add("Staff");
            listHeader.add("Nurse");
