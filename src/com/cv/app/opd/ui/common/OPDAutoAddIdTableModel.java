 ...
 * @author WSwe
 */
public class OPDAutoAddIdTableModel extends AbstractTableModel {

    static Logger log = Logger.getLogger(OPDAutoAddIdTableModel.class.getName());
    private List<OPDAutoAddIdMapping> listService;
    private final String[] columnNames = {"Description", "Qty", "Price", "Sort Order"};
    private final AbstractDataAccess dao = Global.dao;
    private Integer serviceId;

    public OPDAutoAddIdTableModel() {
    }
    ...
    public Object getValueAt(int row, int column) {
        ...
        OPDAutoAddIdMapping record = listService.get(row);
        switch (column) {
            case 0: //Description
        ...
        switch (column) {
            case 0: //Description
                if (value == null) {
                    ...
                    record.getKey().setAddServiceId((Service) value);
                    record.getKey().setTranOption("OPD");
    ...
    public void assignAutoId(Integer serviceId) {
        try {
            if (serviceId != null) {
                ...
                String strSql = "select o from OPDAutoAddIdMapping o "
                        + "where o.key.tranOption = 'OPD' and o.key.tranServiceId = "
                        + serviceId + " order by o.sortOrder";
                listService = dao.findAllHSQL(strSql);
                if (listService == null) {
                    listService = new ArrayList();
                }
                listService.add(new OPDAutoAddIdMapping());
    ...
    public void delete(int row) {
        //Item itm = listSaleDetail.get(row);
        if (listService != null) {
            if (!listService.isEmpty()) {

                OPDAutoAddIdMapping opd = listService.get(row);
                try {
                    dao.execSql("delete from auto_add_id_mapping where tran_option='"
                            + opd.getKey().getTranOption() + "' and tran_service_id="
                            + opd.getKey().getTranServiceId() + " and add_service_id="
                            + opd.getKey().getAddServiceId().getServiceId());
                } catch (Exception ex) {
    ...
    private boolean hasEmptyRow() {
        ...
        if (listService != null) {
            if (!listService.isEmpty()) {
                OPDAutoAddIdMapping srv = listService.get(listService.size() - 1);
                if (srv != null) {
                    if (srv.getKey() != null) {
