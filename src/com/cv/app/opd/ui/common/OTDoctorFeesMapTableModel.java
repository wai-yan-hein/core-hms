 ...
 * @author WSwe
 */
public class OTDoctorFeesMapTableModel extends AbstractTableModel {

    private List<DoctorFeesMappingOT> listDoctorFeesMapping = new ArrayList();
    private final String[] columnNames = {"Service", "Fees"};
    private final AbstractDataAccess dao;
    private String deletedList;

    public OTDoctorFeesMapTableModel(AbstractDataAccess dao) {
        this.dao = dao;
    }
    ...
    public Object getValueAt(int row, int column) {
        DoctorFeesMappingOT record = listDoctorFeesMapping.get(row);
    ...
    public List<DoctorFeesMappingOT> getListDoctorFeesMapping() {
        String strSql = "SELECT * FROM com.cv.app.opd.database.entity.DoctorFeesMappingOT"
                + " WHERE service IS NOT NULL";
        return JoSQLUtil.getResult(strSql, listDoctorFeesMapping);
    }

    public void setListDoctorFeesMapping(List<DoctorFeesMappingOT> listDoctorFeesMapping) {
        this.listDoctorFeesMapping = listDoctorFeesMapping;
        addNewRow();
        fireTableDataChanged();
    }

    public DoctorFeesMappingOT getDoctorFeesMapping(int row) {
        return listDoctorFeesMapping.get(row);
    }

    public void setDoctorFeesMapping(int row, DoctorFeesMappingOT doctorFeesMapping) {
        listDoctorFeesMapping.set(row, doctorFeesMapping);
        fireTableRowsUpdated(row, row);
    }

    public void addDoctorFeesMapping(DoctorFeesMappingOT doctorFeesMapping) {
    ...
    public void deleteDoctorFeesMapping(int row) {
        DoctorFeesMappingOT dfm = listDoctorFeesMapping.get(row);
    ...
    public void addNewRow() {
        ...
        if (count == 0 || listDoctorFeesMapping.get(count - 1).getService() != null) {
            listDoctorFeesMapping.add(new DoctorFeesMappingOT());
        }
    }
    ...
    public boolean isValidEntry() {
        ...
        for (DoctorFeesMappingOT dfm : listDoctorFeesMapping) {
