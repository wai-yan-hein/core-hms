...
import com.cv.app.opd.ui.common.CityTableModel;
...
public class CitySetup extends javax.swing.JDialog {
    ...
    private boolean isValidEntry() {
        ...
        if (Util1.nullToBlankStr(txtCityName.getText()).equals("")) {
            JOptionPane.showMessageDialog(this, "City name must not be blank.",
                    "Blank", JOptionPane.ERROR_MESSAGE);
            status = false;
            txtCityName.requestFocusInWindow();
        } else {
            currCity.setCityName(txtCityName.getText());
        }
        ...
        if (isValidEntry()) {
            try {
                ...
                JOptionPane.showMessageDialog(this, "Duplicate entry.",
                        "City Name", JOptionPane.ERROR_MESSAGE);
                dao.rollBack();
            } catch (Exception ex) {
        ...
        if (lblStatus.getText().equals("EDIT")) {
            try {
                int yes_no = JOptionPane.showConfirmDialog(Util1.getParent(), "Are you sure to delete?",
                        "City Delete", JOptionPane.YES_NO_OPTION);

                if (yes_no == 0) {
                    dao.delete(currCity);
                    int tmpRow = selectRow;
                    selectRow = -1;
                    tableModel.deleteCity(tmpRow);
                }
            } catch (ConstraintViolationException ex) {
