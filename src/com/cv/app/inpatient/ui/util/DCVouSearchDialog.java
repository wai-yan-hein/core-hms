...
import com.cv.app.inpatient.ui.common.DCServiceFilterTableModel;
 ...
 * @author WSwe
 */
public class DCVouSearchDialog extends javax.swing.JDialog implements SelectionObserver {

    static Logger log = Logger.getLogger(DCVouSearchDialog.class.getName());
    private final AbstractDataAccess dao = Global.dao;
    private SelectionObserver observer;
    private VouFilter vouFilter;
    private int selectedRow = -1;
    private TableRowSorter<TableModel> sorter;
    private DCServiceFilterTableModel srvFilterTableModel
            = new DCServiceFilterTableModel(dao, true, "DCVoucherSearch");
    private DCVouTableModel vouTableModel = new DCVouTableModel();
     ...
     */
    public DCVouSearchDialog(SelectionObserver observer, String option) {
    ...
    private void getPrvFilter() {
        try {
            VouFilter tmpFilter = (VouFilter) dao.find("VouFilter", "key.tranOption = 'DCVouSearch'"
                    + " and key.userId = '" + Global.machineId + "'");

            if (tmpFilter == null) {
                vouFilter = new VouFilter();
                vouFilter.getKey().setTranOption("DCVouSearch");
    ...
    private void initTable() {
        try {
            List<DCVouSrvFilter> listVouSrvFilter = dao.findAll("DCVouSrvFilter",
                    "key.tranOption = 'DCVoucherSearch' and key.userId = '"
                    + Global.machineId + "'");
    ...
    private String getHSQL() {
        /*String strSql = "select distinct v from DCHis v join v.listOPDDetailHis h where date(v.invDate) between '"
                + DateUtil.toDateStrMYSQL(txtFrom.getText()) + "' and '"
                + DateUtil.toDateStrMYSQL(txtTo.getText()) + "'";*/
        String strSql = "select distinct date(sh.dc_date) dc_date, sh.dc_inv_id, sh.remark, \n"
                + "sh.patient_id cus_id, sh.patient_name cus_name, sh.package_id, "
                + "usr.user_name, sh.vou_total, sh.deleted, sh.dc_status\n"
                + "from dc_his sh\n"
                + "join dc_details_his sdh on sh.dc_inv_id = sdh.vou_no\n"
                + "join inp_service med on sdh.service_id = med.service_id\n"
                + "join appuser usr on sh.created_by = usr.user_id\n"
                + "where date(sh.dc_date) between '" + DateUtil.toDateStrMYSQL(txtFrom.getText()) + "' and '"
                + DateUtil.toDateStrMYSQL(txtTo.getText()) + "'";
        ...
        if (!txtVouNo.getText().isEmpty()) {
            strSql = strSql + " and sh.dc_inv_id = '" + txtVouNo.getText() + "'";
            vouFilter.setVouNo(txtVouNo.getText());
        } else {
            vouFilter.setVouNo(null);
        }
        if (!txtPtNo.getText().isEmpty()) {
            strSql = strSql + " and sh.patient_id = '" + txtPtNo.getText() + "'";
        } else if (!txtPtName.getText().trim().isEmpty()) {
            strSql = strSql + " and upper(sh.patient_name) like '%" + txtPtName.getText().trim().toUpperCase() + "%'";
        ...
        if (cboMachine.getSelectedItem() instanceof MachineInfo) {
            ...
            strSql = strSql + " and sh.ot_inv_id like '" + machineId + "%'";
        ...
        if (srvFilterTableModel.getFilterCodeStr() != null) {
            strSql = strSql + " and sdh.service_id in ("
                    + srvFilterTableModel.getFilterCodeStr() + ")";
        }

        strSql = strSql + " order by date(sh.dc_date) desc, sh.dc_inv_id desc";
        ...
        /*if(option.equals("RESULT")){
            ...
            for(VService vs : listS){
                if(filterId.isEmpty()){
                    filterId = vs.getGroupId().toString();
                }else{
                    filterId = filterId + "," + vs.getGroupId().toString();
                }
            }
            
            if(!filterId.isEmpty()){
                strSql = strSql + " and h.service.serviceId in (" + filterId + ")";
            }
        }*/
        //Filter history save
        try {
    ...
    private void select() {
        if (selectedRow >= 0) {
            observer.selected("DCVouList",
                    vouTableModel.getOPDHis(tblVou.convertRowIndexToModel(selectedRow)));
            dispose();
        } else {
            JOptionPane.showMessageDialog(Util1.getParent(), "Please select DC voucher.",
  ...
  private void txtPtNoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_txtPtNoActionPerformed
    ...
    private void txtPtNoFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_txtPtNoFocusLost
        // TODO add your handling code here:
        if (txtPtNo.getText() == null || txtPtNo.getText().isEmpty()) {
            txtPtNo.setText(null);
        }
    }//GEN-LAST:event_txtPtNoFocusLost
